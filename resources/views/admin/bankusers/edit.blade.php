        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.bankuser.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.bankusers.update", [$bankuser->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('id_partner') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="id_partner_id" id="id_partner_id" required>
                                @foreach($id_partners as $id => $id_partner)
                                    <option value="{{ $id }}" {{ ($bankuser->id_partner ? $bankuser->id_partner->id : old('id_partner_id')) == $id ? 'selected' : '' }}>{{ $id_partner }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('nama') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="nama_id" id="nama_id" required>
                                @foreach($namas as $id => $nama)
                                    <option value="{{ $id }}" {{ ($bankuser->nama ? $bankuser->nama->id : old('nama_id')) == $id ? 'selected' : '' }}>{{ $nama }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('nama'))
                        ...
                        <div class="form-group {{ $errors->has('nomor_rekening') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="nomor_rekening" id="nomor_rekening" value="{{ old('nomor_rekening', $bankuser->nomor_rekening) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('atas_nama') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="atas_nama" id="atas_nama" value="{{ old('atas_nama', $bankuser->atas_nama) }}" required>
