        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.currencyUser.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.currency-users.update", [$currencyUser->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('id_partner') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="id_partner_id" id="id_partner_id" required>
                                @foreach($id_partners as $id => $id_partner)
                                    <option value="{{ $id }}" {{ ($currencyUser->id_partner ? $currencyUser->id_partner->id : old('id_partner_id')) == $id ? 'selected' : '' }}>{{ $id_partner }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('nama') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="nama_id" id="nama_id" required>
                                @foreach($namas as $id => $nama)
                                    <option value="{{ $id }}" {{ ($currencyUser->nama ? $currencyUser->nama->id : old('nama_id')) == $id ? 'selected' : '' }}>{{ $nama }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('nama'))
                        ...
                        <div class="form-group {{ $errors->has('no_account') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="no_account" id="no_account" value="{{ old('no_account', $currencyUser->no_account) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('nama_account') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="nama_account_id" id="nama_account_id">
                                @foreach($nama_accounts as $id => $nama_account)
                                    <option value="{{ $id }}" {{ ($currencyUser->nama_account ? $currencyUser->nama_account->id : old('nama_account_id')) == $id ? 'selected' : '' }}>{{ $nama_account }}</option>
                                @endforeach
                            </select>
