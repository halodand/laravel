...
class BankController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $query = Bank::with(['team'])->select(sprintf('%s.*', (new Bank)->table));
            ...
            $table->editColumn('atasnama', function ($row) {
                return $row->atasnama ? $row->atasnama : "";
            });
            $table->editColumn('status', function ($row) {
                return $row->status ? Bank::STATUS_RADIO[$row->status] : '';
            });
